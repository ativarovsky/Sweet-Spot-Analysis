...
### Feb 2024 New vs Established

Subset to February
```{r}
tbl_mms01_feb_2024 <- 
  tbl_mms01 |> 
  filter(mos == 202402 & company_id %in% c('ALTAMED', 'ALTAOC', 'ALTACHLA'))
...
plot_cost_by_month <- 
  claim_header_by_patient |> 
  filter(month_year_from >= '2024-02-01') |> 
  left_join(tbl_mms01_feb_2024 |> select(c(memberid, mms_new)), by = c("member_id" = "memberid")) |> 
  filter(!is.na(mms_new)) |> 
  mutate(mms_new = as.factor(mms_new)) |> 
  group_by(mms_new, month_year_from) |> 
  summarise(average_cost = mean(total_cost)) 
...
plot_cost_by_month |> 
  ...
  geom_point(aes(color = mms_new)) + 
  geom_smooth(aes(color = mms_new), se = FALSE) + 
  labs(title = "Feb 2024 New vs Existing Patients", 
       subtitle = "Avg Cost over Time")
...
plot_cost_by_month <- 
  claim_header_by_patient |> 
  filter(month_year_from >= '2024-03-01') |> 
  left_join(tbl_mms01_mar_2024 |> select(c(memberid, mms_new)), by = c("member_id" = "memberid")) |> 
  filter(!is.na(mms_new)) |> 
  mutate(mms_new = as.factor(mms_new)) |> 
  group_by(mms_new, month_year_from) |> 
...
plot_cost_by_month |> 
  ...
  geom_point(aes(color = mms_new)) + 
  geom_smooth(aes(color = mms_new), se = FALSE) + 
  labs(title = "Mar 2024 New vs Existing Patients", 
       subtitle = "Avg Cost over Time")
...
### Apr 2024 New vs Established

Subset to April
```{r}
tbl_mms01_apr_2024 <- 
  tbl_mms01 |> 
  filter(mos == 202404 & company_id %in% c('ALTAMED', 'ALTAOC', 'ALTACHLA'))
...
plot_cost_by_month <- 
  claim_header_by_patient |> 
  filter(month_year_from >= '2024-04-01') |> 
  left_join(tbl_mms01_apr_2024 |> select(c(memberid, mms_new)), by = c("member_id" = "memberid")) |> 
  filter(!is.na(mms_new)) |> 
  mutate(mms_new = as.factor(mms_new)) |> 
  group_by(mms_new, month_year_from) |> 
  summarise(average_cost = mean(total_cost)) 
...
plot_cost_by_month |> 
  ...
  geom_point(aes(color = mms_new)) + 
  geom_smooth(aes(color = mms_new), se = FALSE) + 
  labs(title = "Apr 2024 New vs Existing Patients", 
       subtitle = "Avg Cost over Time")
...
### May 2024 New vs Established

Subset to May
```{r}
tbl_mms01_may_2024 <- 
  tbl_mms01 |> 
  filter(mos == 202405 & company_id %in% c('ALTAMED', 'ALTAOC', 'ALTACHLA'))
...
plot_cost_by_month <- 
  claim_header_by_patient |> 
  filter(month_year_from >= '2024-05-01') |> 
  left_join(tbl_mms01_may_2024 |> select(c(memberid, mms_new)), by = c("member_id" = "memberid")) |> 
  filter(!is.na(mms_new)) |> 
  mutate(mms_new = as.factor(mms_new)) |> 
  group_by(mms_new, month_year_from) |> 
  summarise(average_cost = mean(total_cost)) 
...
plot_cost_by_month |> 
  ...
  geom_point(aes(color = mms_new)) + 
  geom_smooth(aes(color = mms_new), se = FALSE) + 
  labs(title = "May 2024 New vs Existing Patients", 
       subtitle = "Avg Cost over Time")
...
##  Import Tables (Updated)

### Jun 2024 Start

Per Steve Wong: patient table and company tables are produced below. 

First, define date of interest (month and year that you want to see new members
```{r}
...
# join tables
today_and_future_elig_fq_members <- 
  memb_hphists |> 
...
unique_dwh_ids <- 
  dim_all_patient |> 
  inner_join(today_and_future_elig_fq_members, by = c("all_patient_id" = "memb_mpi_no")) |> 
...
```{r}
members_seen_in_system_prior_to_current_month_load <- 
  unique_dwh_ids |> 
...
net_new_members_jun_24 <- 
  today_and_future_elig_fq_members |> 
  left_join(members_seen_in_system_prior_to_current_month_load, by = c("memb_mpi_no" = "all_patient_id")) |> 
  filter(is.na(current_patient_sid)) |> 
...
claim_header <- 
  ...
  tbl(in_schema("data", "tblClaim_Header")) |> 
  select(c(ClaimNo, DateCreated, DateFrom, MemberId, CompanyId, TotalNet, PlaceofSvcCode, StatusCode)) |> 
  filter(DateFrom >= date_of_interest_start & DateFrom < claims_cutoff) |> 
  filter(CompanyId %in% c("ALTACHLA", "ALTAMED", "ALTANET", "ALTAOC") & StatusCode == '9') |> 
  collect() |> 
  janitor::clean_names() |> 
  filter(!is.na(claim_no))
...
Plot cost curves for new vs existing patients, Jun 2024
```{r}
grouped_cost_by_month_patient_status <- 
  cost_by_month_jun_2024 |> 
  select(c(month_year_from, total_cost, new_patient)) |> 
